...
dt_1s = np.arange(0, 21, 1)
dt_2s = np.arange(0, 21, 2)

velocity_1 = v_f + (v_o - v_f) * np.exp((- c/m)*dt_1s)
velocity_2 = v_f + (v_o - v_f) * np.exp((- c/m)*dt_2s)

#Part a)
Integration_1 = (integrate_newton(dt_1s,velocity_1,'trap'))
...
Integration_3 = (integrate_newton(dt_1s,velocity_1,'simp'))
